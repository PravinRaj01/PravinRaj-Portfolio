...
import { GraduationCap, Award, Calendar, MapPin, ArrowUpDown, ExternalLink, Star } from 'lucide-react';
...
const Academic = () => {
  ...
  useEffect(() => {
    if (siteSettings?.academic_default_view) {
      setViewMode(siteSettings.academic_default_view);
    }
  }, [siteSettings?.academic_default_view]);
  ...
  // Sort certifications: featured first (in cards view), then by date
  const sortedCertifications = useMemo(() => {
    ...
    return sorted;
  }, [certifications, sortOrder, viewMode]);
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 md:gap-6">
                {sortedCertifications.map((cert, index) => (
                  <motion.div
                    ...
                    transition={{ duration: 0.6, delay: index * 0.1 }}
                    viewport={{ once: true }}
                    className={cert.featured ? 'relative' : ''}
                  >
                    {/* Featured animated glow effect - only in cards view */}
                    {cert.featured && (
                      <div 
                        className="absolute -inset-[2px] rounded-xl bg-gradient-to-r from-primary via-purple-500 to-primary opacity-75 blur-sm animate-pulse"
                        style={{
                    ...
                    <Card className={`h-full hover:shadow-lg transition-shadow relative ${
                      cert.featured ? 'bg-background' : ''
                    }`}>
                      {/* Featured Badge */}
                      {cert.featured && (
                        <Badge 
                          className={`absolute top-2 right-2 z-10 text-[10px] sm:text-xs bg-gradient-to-r ${
                            mode === 'creative' 
                              ? 'from-orange-500 to-orange-600' 
                              : 'from-blue-600 to-purple-600'
                          } text-white`}
                      ...
                      <CardHeader className="p-3 sm:p-4 md:p-6">
                        <CardTitle className="text-sm sm:text-base md:text-lg pr-16">{cert.name}</CardTitle>
