      ...
      <div className="hidden lg:block">
        ...
        >
          {projects.map((project) => (
            <SwiperSlide
              key={project.id}
              className="quote-outer-container bg-card text-card-foreground flex flex-row justify-between rounded-2xl p-8 lg:p-12 text-left"
            >
              {/* Left Content */}
              ...
              {/* Right Content - Image with hover scroll effect */}
              <div className="relative h-[22rem] overflow-hidden rounded-xl w-[40%] shadow-2xl group">
                {project.image_drive_id ? (
                  <img
                    src={generateDriveImageUrl(project.image_drive_id, 'large')}
                    alt={`${project.title} mockup`}
                    className="w-full h-auto transition-all duration-[6000ms] transform hover:translate-y-[-50%]"
                  />
                ) : (
