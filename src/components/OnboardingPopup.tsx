...
const OnboardingPopup = () => {
  ...
  const { mode, handleModeToggle, professionalModeEnabled, creativeModeEnabled, modeSettingsLoaded } = usePortfolio();

  // Only show onboarding if BOTH modes are enabled
  const bothModesEnabled = professionalModeEnabled && creativeModeEnabled;

  useEffect(() => {
    // Wait for settings to load before deciding to show popup
    if (!modeSettingsLoaded) return;
  ...
  }, [bothModesEnabled, modeSettingsLoaded]);
